...
- CORS 에러
  - 에러
    - cors 패키지를 추가하고 CORS 관련 헤더를 모든 도메인에서 요청이 허용하도록 처리을 해줬지만 CORS 정책에 의해 차단되었다는 오류가 콘솔에서 발생
      - 콘솔에서 에러가 발생하기 전, 프론트엔드 코드의 fetch 함수 내용에 `credentials: "include"` 라는 코드를 추가해 제어하려고 했지만 정상적으로 제어하지 못하고 에러 발생
  - 해결 방법
    - 백엔드의 CORS 관련 코드에서 origin과 credentials 내용 추가해 구성
      - origin
        - 프론트엔드 내용을 출력하는 localhost URL 경로를 지정
        - 나중에는 배포 환경 URL 경로를 지정
      - credentials
        - `credentials: true`로 설정
    - 프론트엔드와 백엔드에서 모두 `credentials`를 추가해 정상적으로 인증 정보가 교환 되도록 구성
      - 한 쪽이라도 설정되지 않으면 브라우저는 보안상의 이유로 쿠키를 전송하지 않음
  - cors 설정에 대한 추가 정리
    - origin
      - 서버에 요청을 보내는 클라이언트 출처를 나타냄
      - origin 옵션을 사용하여 요청이 허용되는 도메인을 제한할 수 있음
      - ex) `"http://localhost:5173"`과 같은 특정한 도메인을 명시할 수 있음
        - 배포 환경에서는 배포되는 사이트 URL을 지정
      - 모든 도메인을 허용하려면 `*`을 사용할 수 있지만, 보안상의 이유로 권장되지 않음
    - credentials
      - 보통 로그인 정보나 세션 쿠키를 포함한 요청을 보낼 때 사용
      - 해당 옵션을 사용하면 서버가 요청의 인증 정보를 확인하고 해당 요청을 처리할 수 있음
      - 프론트엔드의 credentials
        - `credentials: include`
        - 요청이 CORS를 통해 전송될 때, 인증 정보(쿠키 등)를 함께 보내도록 브라우저에 지시
      - 백엔드의 credentials
        - `credentials: true`
        - 클라이언트에서 보낸 요청이 인증 정보를 포함하고 있음을 허용
